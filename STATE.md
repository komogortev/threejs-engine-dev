...
Working: Phase 3d signed off (2026-04-12). Player locomotion, camera presets, sandbox, settings. **D-0–D-6 complete** (see Decision Log for per-step detail). **Post-D — Named scene persistence (2026-05-24):** `assetDb` bumped to Dexie v2 with `scenes: 'id, name, savedAt'` table + `SceneRow` type. Editor "Save Scene" button with name `<input>` writes to `assetDb.scenes`; `currentSceneId` tracks current scene for overwrite vs. new. `SandboxSceneModule.loadPlacedObjects(sceneId)` reads from `assetDb.scenes`. `SandboxView` shows scene picker overlay before engine mounts; auto-migrates `localStorage['sandbox:scene']` to Dexie on first visit. `MenuView` gate checks Dexie count + legacy localStorage key. Browser-verified: migration → picker → load end-to-end.
...
Next: **Phase D complete + named scene persistence done.** Next: T-F7 GLB model normalization (pipeline + runtime), or Phase 3 editor writability (F-2/F-9/F-10/F-11/F-13).
