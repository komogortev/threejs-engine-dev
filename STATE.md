...
## SNAPSHOT
Phase: Phase D COMPLETE → Phase 5 Room Package pipeline | Last: 2026-05-30 | Stack: Vue 3 + @base Three.js harness
...
Next: **Phase 5 Room Package pipeline (J-1 close) — 3-session plan locked 2026-05-30.** S1: Dexie v3 (`SceneRow.config?: SceneEditorConfig`) + `'audio'` AssetKind + `SceneEditorConfig.ambientAudioAssetId/Volume` + inspector ambient audio picker. S2: `roomPackageTypes.ts` (`RoomPackageManifest` + `RoomPackageScene`) + `exportRoomPackage.ts` + `buildRoomPackageScene` + "Export Room Package" button in editor toolbar. S3: `loadRoomPackage.ts` (SHARED) + `RoomPlayerView.vue` + `RoomPlayerModule.ts` + `/room` route + `MusicLayer` ambient wiring. Deliverable: author a room in editor → Export ZIP → drag onto /room → walk + hear ambient audio. T-F7 GLB normalization follows (unblocks P3b Rodin character assets).
