...
Next: Critical path — scene editor harmonization → camera architecture review → Phase 3d sign-off (see `PLAN-critical-path.md`)
...
- **Next up (critical path):** Harness scene editor harmonization — `SceneEditorPage.vue` → scenes prop + sandbox + registry (mirrors three-dreams pattern)
- **Then:** Camera architecture review + documentation → Phase 3d sign-off
