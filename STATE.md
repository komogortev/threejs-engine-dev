...
## SNAPSHOT
Phase: Phase D — D-0 ✅, D-1 ✅, D-2 ✅, D-3 next | Last: 2026-05-21 | Stack: Vue 3 + @base Three.js harness
Working: Phase 3d signed off (2026-04-12). Player locomotion, camera presets, sandbox, settings. **D-0 complete:** menu collapsed to Sandbox · Editor · Settings; dead routes (/game /dbox /scene /scene-editor) removed; GameView/DboxView/SceneView/DboxSceneModule/dbox.ts deleted; Sandbox disabled until saved scene exists (localStorage `sandbox:scene`); legacy `/editor` stays as frozen Phase 4 reference; WaypointEditorPage back link → `/editor`. **D-1 complete:** `'free-float'` added to `GameplayCameraMode` union (`@base/camera-three`); `PlayerCameraCoordinator` (`@base/gameplay`) extended with `ffYaw`/`ffPitch`/`ffPosition` state, pitch-aware YXZ WASD movement, `lastCamera` seeding fallback; Tab cycles third→first→free-float; browser-verified. **D-2 complete:** `/editor` route now hosts `SceneEditorPage.vue` (from `@base/ui` via local host page); legacy `EditorView.vue` parked at `/editor-legacy` for piecewise migration reference (D-5). Build clean (5.09s).
...
Next: **D-3 — TransformControls** in `useSceneEditorViewport` (= F-LE1 early migration).
