...
export function loadHeightmap(
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      if (!ctx2d) { reject(new Error('Canvas 2D unavailable')); return }

      ctx2d.drawImage(img, 0, 0)
      const { data } = ctx2d.getImageData(0, 0, img.width, img.height)

      // Decode: average R+G+B channels → brightness [0,1] → normalise to [-1,1]
      // Mid-grey (128,128,128) → 0.502 → ~0.004 ≈ 0 (acceptable float rounding)
 ...
 *
 * UV convention:
 *   u=0 → world −X edge   u=1 → world +X edge
 *   v=0 → world −Z edge   v=1 → world +Z edge  (image top = world north = −Z)
 */
export function sampleHeightmap(data: HeightmapData, x: number, z: number): number {
  ...
  // Four surrounding pixels, bilinear blend
